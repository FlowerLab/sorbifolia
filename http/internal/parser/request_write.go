...
func (r *RequestWriter) parseBody(p []byte) (n int, err error) {
...
func (r *RequestWriter) parseHeader(p []byte) (n int, err error) {
	var (
...
func (r *RequestWriter) parseMethod(p []byte) (n int, err error) {
	var (
...
func (r *RequestWriter) parseURI(p []byte) (n int, err error) {
	var (
		i    = bytes.IndexByte(p, char.Space) // Method URI HTTP/1.1
		buf  = &r.buf
		is09 = false
...
func (r *RequestWriter) parseVersion(p []byte) (n int, err error) {
	var (
...
func (r *RequestWriter) Read(p []byte) (n int, err error) {
	if r.state != END {
