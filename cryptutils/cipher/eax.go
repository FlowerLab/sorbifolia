...
const (
	eaxDefaultTagSize   = 16
	eaxDefaultNonceSize = 16
)
...
// NewEAX returns an EAX instance. Supports 128, 192, 256 - bit key length.
func NewEAX(block cipher.Block) (cipher.AEAD, error) {
	return NewEAXWithNonceAndTagSize(block, eaxDefaultNonceSize, eaxDefaultTagSize)
}

// NewEAXWithNonceAndTagSize returns an EAX instance with given nonce and tag
// lengths in bytes.
//
...
func NewEAXWithNonceAndTagSize(block cipher.Block, nonceSize, tagSize int) (cipher.AEAD, error) {
	if nonceSize < 1 {
		return nil, errNonceSizeTooSmall
	}
	if tagSize > block.BlockSize() {
		return nil, errTagLengthExceedsBlockSize
	}
	return &eax{
		block:     block,
		tagSize:   tagSize,
...
func (e *eax) Open(dst, nonce, ciphertext, adata []byte) ([]byte, error) {
	...
	if len(ciphertext) < e.tagSize {
		return nil, errCiphertextShorterThanTagLength
	}
	...
	if subtle.ConstantTimeCompare(ciphertext[sep:], tag) != 1 {
		return nil, errTagAuthenticationFailed
	}
...
var (
	errNonceSizeTooSmall              = errors.New("eax: nonce size too small")
	errTagLengthExceedsBlockSize      = errors.New("eax: tag length exceeds block size")
	errCiphertextShorterThanTagLength = errors.New("eax: ciphertext shorter than tag length")
	errTagAuthenticationFailed        = errors.New("eax: tag authentication failed")
	errNonceTooLong                   = "eax: nonce too long for this instance"
)
