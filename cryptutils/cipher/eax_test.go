...
func TestEaxEncryptDecryptVectorsWithPreviousDataRandomizeSlow(t *testing.T) {
	...
	for _, keyLength := range allowedKeyLengths {
		...
		nonce := make([]byte, 1+mathRand.Intn(eaxBlockLength))
		previousData := make([]byte, mathRand.Intn(eaxMaxLength-2*eaxBlockLength))
...
func TestEaxRejectTamperedCiphertextRandomizeSlow(t *testing.T) {
	...
	nonce := make([]byte, eaxBlockLength)
...
func TestEaxParameters(t *testing.T) {
	...
	t.Run("Should not give error with allowed custom parameters", func(st *testing.T) {
		...
		tagSize := 12 + mathRand.Intn(eaxBlockLength-11)
		aesCipher, err := aes.NewCipher(key)
		if err != nil {
			t.Fatal(err)
		}
...
func BenchmarkEaxEncrypt(b *testing.B) {
	headerLength := 16
	pt := make([]byte, eaxMaxLength)
...
func BenchmarkEaxDecrypt(b *testing.B) {
	headerLength := 16
	pt := make([]byte, eaxMaxLength)
	header := make([]byte, headerLength)
	key := make([]byte, eaxBlockLength)
