...
func TestOcbZeroHash(t *testing.T) {
	...
	o := ocb{
		block:     aesCipher,
		tagSize:   ocbDefaultTagSize,
		nonceSize: ocbDefaultNonceSize,
	}
...
func TestOcbEncryptDecryptVectorsWithPreviousDataRandomizeSlow(t *testing.T) {
	...
	for _, keyLength := range allowedKeyLengths {
		...
		nonce := make([]byte, 1+mathrand.Intn(ocbBlockLength-1))
		previousData := make([]byte, mathrand.Intn(ocbMaxLength))
...
func TestOcbRejectTamperedCiphertextRandomizeSlow(t *testing.T) {
	...
	// Note: Nonce cannot equal ocbBlockLength
...
// shared across tests.
var ocbTestKey, _ = hex.DecodeString("000102030405060708090A0B0C0D0E0F")
